...
def spectral_centroid(magnitude_spectrum: torch.Tensor, magnitude_freqs: torch.Tensor):
    ...
    centroid[torch.isinf(centroid)] = 0.0
    centroid[torch.isneginf(centroid)] = 0.0
    return centroid
...
def spectral_entropy(power_spectrum: torch.Tensor):
    ...
    entropy[torch.isneginf(entropy)] = 0.0       
    return entropy
...
def spectral_flatness(magnitude_spectrum: torch.Tensor):
    ...
    flatness[torch.isnan(flatness)] = 0.0
    flatness[torch.isinf(flatness)] = 0.0
    flatness[torch.isneginf(flatness)] = 0.0
    return flatness
...
def spectral_moments(audio, magnitude_freqs: torch.Tensor):
    ...
    spectral_variance[torch.isnan(spectral_variance)] = 0.0
    spectral_variance[torch.isinf(spectral_variance)] = 0.0
    spectral_variance[torch.isneginf(spectral_variance)] = 0.0
    spectral_skewness[torch.isnan(spectral_skewness)] = 0.0
    spectral_skewness[torch.isinf(spectral_skewness)] = 0.0
    spectral_skewness[torch.isneginf(spectral_skewness)] = 0.0
    spectral_kurtosis[torch.isnan(spectral_kurtosis)] = 0.0
    spectral_kurtosis[torch.isinf(spectral_kurtosis)] = 0.0
    spectral_kurtosis[torch.isneginf(spectral_kurtosis)] = 0.0
...
def spectral_roll_off_point(power_spectrum: torch.Tensor, magnitude_freqs: torch.Tensor, n):
    ...
    roll_offs[torch.isnan(roll_offs)] = 0.0
    roll_offs[torch.isinf(roll_offs)] = 0.0
    roll_offs[torch.isneginf(roll_offs)] = 0.0
    return roll_offs
...
def spectral_slope(magnitude_spectrum: torch.Tensor, magnitude_freqs: torch.Tensor):
    ...
    slopes[torch.isnan(slopes)] = 0.0
    slopes[torch.isinf(slopes)] = 0.0
    slopes[torch.isneginf(slopes)] = 0.0
    y_ints[torch.isnan(y_ints)] = 0.0
    y_ints[torch.isinf(y_ints)] = 0.0
    y_ints[torch.isneginf(y_ints)] = 0.0
